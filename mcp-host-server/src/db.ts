...
export function getHostDb(): Client {
  if (!_db) {
...
export async function mcpGet<T = Record<string, unknown>>(sql: string, args: unknown[] = []): Promise<T | undefined> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const result = await getHostDb().execute({ sql, args: args as any[] });
  return result.rows.length === 0 ? undefined : result.rows[0] as T;
}

export async function mcpRun(sql: string, args: unknown[] = []): Promise<void> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
