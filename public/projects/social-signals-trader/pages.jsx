...
function SleeveActivation({ rows }) {
  const [showDormant, setShowDormant] = React.useState(false);
  const list = Array.isArray(rows) ? rows : [];
  if (!list.length) return null;

  const visible = showDormant ? list : list.filter(r => !r.dormant);
  const activeCount = list.filter(r => !r.dormant).length;
      ...
      <div className="card-body tight">
        <div style={{ marginBottom: 8 }}>
          <button
            ...
            style={{ background: "none", border: "1px solid var(--line)", borderRadius: 3, padding: "3px 8px", cursor: "pointer", color: "var(--text-2)", fontSize: 11 }}
          >
            {showDormant ? "Hide dormant" : `Show all (incl. ${dormantCount} dormant)`}
          </button>
        </div>
        <table className="trades">
          <thead>
          ...
          <tbody>
            {visible.map(r => (
              <tr key={r.id} style={r.dormant ? { opacity: 0.5 } : undefined}>
                <td className="ticker">{r.id}</td>
                <td className="dim" style={{ maxWidth: 240 }}>{r.name}</td>
                <td className="mono" style={{ fontSize: 11 }}>{r.status}</td>
                <td className="num">{(Number(r.capital_pct) * 100).toFixed(0)}%</td>
                <td className="num">{r.n_open}</td>
                <td className="num">{r.n_closed}</td>
                <td className="dim mono" style={{ fontSize: 11 }}>{r.first_trade_date || "—"}</td>
                <td className="dim mono" style={{ fontSize: 11 }}>{r.last_trade_date || "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function UniverseCoverage({ data }) {
  const [signalOpen, setSignalOpen] = React.useState(false);
  const [gapOpen, setGapOpen] = React.useState(false);

  if (!data) return null;
  const {
    scan_ts, universe_size = 500, scanned = 0,
    with_calendar = 0, with_signal = 0, missing_calendar = 0,
    by_strategy = {}, sample_signaled = [], sample_gaps = [],
  } = data;

  const freshness = scan_ts ? (() => {
    const ms = Date.now() - new Date(scan_ts).getTime();
    const m = Math.round(ms / 60000);
    return m < 60 ? `${m}m ago` : `${Math.round(m / 60)}h ago`;
  })() : "no scan yet";

  const maxStrat = Math.max(1, ...Object.values(by_strategy).map(Number));
    ...
    <div className="card" style={{ marginTop: 12 }}>
      ...
      <div className="card-body" style={{ padding: "12px 16px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 16 }}>
          {[
            { label: "scanned", value: `${scanned} / ${universe_size}` },
            { label: "with calendar", value: with_calendar },
            { label: "with signal", value: with_signal },
            { label: "gaps (dark)", value: missing_calendar },
          ].map(({ label, value }) => (
        ...
        <div style={{ marginBottom: 12 }}>
          <div className="mono" style={{ fontSize: 10, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>signals by strategy</div>
          {Object.entries(by_strategy).map(([strat, count]) => (
            <div key={strat} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <div className="mono" style={{ width: 72, fontSize: 11 }}>{strat}</div>
              <div style={{ flex: 1, background: "var(--surface-2)", borderRadius: 2, height: 8, overflow: "hidden" }}>
                <div style={{ width: `${(Number(count) / maxStrat) * 100}%`, height: "100%", background: "var(--info)", borderRadius: 2 }} />
              </div>
              <div className="mono" style={{ width: 28, fontSize: 11, textAlign: "right" }}>{count}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ borderTop: "1px solid var(--line)", paddingTop: 10 }}>
          <button
            ...
            style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-2)", fontSize: 12, padding: 0, marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}
          >
            <span>{signalOpen ? "▾" : "▸"}</span>
            <span>Top {Math.min(sample_signaled.length, 20)} signaled tickers</span>
          </button>
          {signalOpen && (
            <table className="trades" style={{ marginTop: 6 }}>
              <thead>
                <tr>
                  <th>Ticker</th>
                  <th>Strategies</th>
                  <th className="num">Mentions 7d</th>
                  <th className="num">Days to earnings</th>
                </tr>
              </thead>
              <tbody>
                {sample_signaled.map((r) => (
                  <tr key={r.ticker}>
                    <td className="ticker">{r.ticker}</td>
                    <td className="mono" style={{ fontSize: 11 }}>{(r.strategies || []).join(", ")}</td>
                    <td className="num">{r.mentions_7d ?? "—"}</td>
                    <td className="num">{r.days_to_earnings ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={{ borderTop: "1px solid var(--line)", paddingTop: 10, marginTop: 6 }}>
          ...
          >
            <span>{gapOpen ? "▾" : "▸"}</span>
            <span>Top {Math.min(sample_gaps.length, 20)} invisible tickers (no calendar + no mentions)</span>
          </button>
          {gapOpen && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 6 }}>
              {sample_gaps.map((t) => (
                <span key={t} className="mono" style={{ background: "var(--surface-2)", padding: "2px 6px", borderRadius: 3, fontSize: 11 }}>{t}</span>
              ))}
            </div>
          )}
        </div>
...
function WatchlistPage() {
  ...
  return (
    <main className="shell">
      ...
      {/* UNIVERSE COVERAGE — proactive S&P 500 scan results */}
      <UniverseCoverage data={window.UNIVERSE_COVERAGE} />
    </main>
  );
}
...
function MethodologyPage() {
  ...
  return (
    <main className="shell">
      ...
      {/* SLEEVE ACTIVATION (M3) — which strategies have ever traded */}
      <SleeveActivation rows={window.SLEEVE_ACTIVATION} />
