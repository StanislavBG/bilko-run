...
// scripts/project-pages/dist/renderer/renderer.cjs, built by
...
// stamps — Date.now() is unavailable in some execution contexts that may
// drive this CLI, so this script never generates its own timestamp.
//
// Writes <output dir>/home.html, marketing.html, feature.html,
// architecture.html, and manifest.json ({ generatedAt }).
//
// Path arguments are used as-is (no allowedRoots validation) — this is a
...
//
// Ported from ~/Projects/session-manager/scripts/render-project-pages.cjs
// (PRD 745) — bundle path adapted to this repo's dist/renderer/renderer.cjs
...
function main() {
  ...
  if (!summaryPath || !picksPath || !outDir || !generatedAt) {
    fail('usage: node scripts/project-pages/render.cjs <summary.json> <picks.json> <output dir> <generatedAt>');
  }
  if (Number.isNaN(Date.parse(generatedAt))) {
    fail(`generatedAt must be an ISO 8601 timestamp, got: ${generatedAt}`);
  }

  const bundlePath = path.join(__dirname, 'dist', 'renderer', 'renderer.cjs');
