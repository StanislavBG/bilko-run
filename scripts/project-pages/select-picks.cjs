#!/usr/bin/env node
// Thin CLI wrapper around scoreVariants/mergePicks (scripts/project-pages/
// lib/select.ts), compiled into scripts/project-pages/dist/logic/logic.cjs
// by `npm run build:project-pages-logic` (same bundle validate-summary.cjs
// uses — extended in place rather than adding a third bundle).
//
// Usage:
//   node scripts/project-pages/select-picks.cjs <summary.json> <existing picks.json or 'none'> <output picks.json> [comma-separated resetSlots]
//
...
//
// Ported from
// ~/Projects/session-manager/scripts/select-project-pages-picks.cjs
...
function fail(message) {
  console.error(`select-picks: ${message}`);
  process.exit(1);
}
...
function main() {
  ...
  if (!summaryPath || !existingPicksArg || !outPath) {
    fail('usage: node scripts/project-pages/select-picks.cjs <summary.json> <existing picks.json | none> <output picks.json> [resetSlots]');
  }
