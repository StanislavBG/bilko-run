        ...
        const req = protocol.get(
          ...
          (res) => {
            if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
              ...
              const rReq = rProtocol.get(redirectUrl.toString(), { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; PageRoast/1.0)' }, rejectUnauthorized: false } as any, (rRes) => {
                let data = '';
                rRes.on('data', (c: Buffer) => data += c.toString());
...
Respond ONLY with valid JSON matching this exact schema — no markdown, no extra text:
{
  ...
  app.post('/api/demos/page-roast/compare', async (req, reply) => {
    ...
    try { parsedUrlA = parseUrl(rawUrlA); } catch { reply.status(400); return { error: 'Invalid URL A.' }; }
    try { parsedUrlB = parseUrl(rawUrlB); } catch { reply.status(400); return { error: 'Invalid URL B.' }; }

    async function fetchPage(parsedUrl: URL): Promise<string> {
      const html = await new Promise<string>((resolve, reject) => {
        const protocol = parsedUrl.protocol === 'https:' ? https : http;
        const req = protocol.get(
          parsedUrl.toString(),
          { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; PageRoast/1.0)' }, rejectUnauthorized: false, timeout: 15000 } as any,
          (res) => {
            if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
              const redirectUrl = new URL(res.headers.location, parsedUrl.toString());
              const rProtocol = redirectUrl.protocol === 'https:' ? https : http;
