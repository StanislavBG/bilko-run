...
const CATEGORY_LABELS: Record<string, { label: string; color: string }> = {
  'build-log': { label: 'Build Log', color: 'bg-fire-50 text-fire-600' },
  'lessons': { label: 'Lessons', color: 'bg-blue-50 text-blue-600' },
  'deep-dive': { label: 'Deep Dive', color: 'bg-purple-50 text-purple-600' },
  'market': { label: 'Market', color: 'bg-green-50 text-green-600' },
  'product': { label: 'Product', color: 'bg-yellow-50 text-yellow-600' },
};
  ...
  return (
    <article className="max-w-2xl mx-auto px-6">
      ...
      <header className="pt-16 pb-12 md:pt-24 md:pb-16">
        <h1 className="text-display-xl text-warm-900 animate-slide-up">
          Building in public.
          <br />
        ...
        <p className="mt-5 text-lg text-warm-600 leading-relaxed animate-slide-up" style={{ animationDelay: '60ms' }}>
      ...
      {/* Announcement Card */}
      <div className="bg-gradient-to-r from-fire-50/80 to-warm-50 rounded-2xl shadow-elevation-1 p-6 md:p-8 mb-12 animate-slide-up" style={{ animationDelay: '180ms' }}>
      ...
      {!loading && posts.length > 0 && (
        <div className="space-y-5 mb-16 stagger-children">
          {posts.map(post => {
            const cat = CATEGORY_LABELS[post.category] ?? { label: post.category, color: 'bg-warm-50 text-warm-500' };
            return (
              <Link key={post.slug} to={`/blog/${post.slug}`}
                className="group block bg-white rounded-2xl shadow-elevation-1 p-6 hover:shadow-elevation-2 hover:-translate-y-0.5 transition-all">
                <div className="flex items-center gap-2.5 mb-3">
                  <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md ${cat.color}`}>{cat.label}</span>
                  <span className="text-xs text-warm-400">{formatDate(post.published_at)}</span>
                </div>
                <h2 className="text-lg font-bold text-warm-900 group-hover:text-warm-950 transition-colors mb-2">
                  {post.title}
                </h2>
                <p className="text-sm text-warm-600 leading-relaxed line-clamp-3">{post.excerpt}</p>
                <p className="mt-3 text-sm font-semibold text-warm-600 group-hover:text-warm-800 transition-colors">
                  Read more &rarr;
                </p>
              </Link>
            );
      ...
      {/* What to expect */}
      <section className="border-t border-warm-200/40 py-14 mb-8">
        <h2 className="text-display-md text-warm-900 mb-6">What you'll find here</h2>
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            <div key={label} className="bg-white rounded-xl p-4 shadow-elevation-1">
              <span className="text-lg">{icon}</span>
              <h3 className="font-bold text-warm-800 text-sm mt-1.5">{label}</h3>
      ...
      {/* Newsletter signup */}
      <section className="border-t border-warm-200/40 py-14 mb-8">
        <div className="bg-gradient-to-r from-fire-50/80 to-warm-50 rounded-2xl shadow-elevation-1 p-6 text-center">
          <h3 className="text-lg font-black text-warm-900 mb-2">Get new posts in your inbox</h3>
          <p className="text-sm text-warm-600 mb-5">Weekly lessons from building AI tools solo. No spam. Unsubscribe anytime.</p>
          <form onSubmit={(e) => {
            e.preventDefault();
            ...
            <input type="email" placeholder="you@example.com" required
              className="flex-1 px-4 py-2.5 rounded-xl bg-white shadow-elevation-1 text-sm focus:outline-none focus:ring-2 focus:ring-fire-400" />
            <button type="submit" className="px-5 py-2.5 bg-fire-500 hover:bg-fire-600 text-white font-bold text-sm rounded-xl shadow-md shadow-fire-500/20 transition-all">
              Subscribe
            </button>
          </form>
          <p className="text-xs text-warm-400 mt-3">Join the builders. No marketing fluff.</p>
        </div>
      </section>

      {/* Podcast teaser */}
      <section className="border-t border-warm-200/40 py-14 mb-8">
        <div className="bg-warm-900 rounded-2xl p-7 text-center shadow-dark-elevation">
