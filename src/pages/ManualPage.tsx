    ...
    <main className="mx-auto max-w-6xl px-6 py-12">
      <header className="border-b border-warm-200 pb-8">
        ...
        <ul className="mt-4 flex flex-wrap gap-x-5 gap-y-1 text-sm text-warm-700">
          ...
          {freeChapters.length > 0 && <li className="text-emerald-700">{freeChapters.length} readable free, right now</li>}
          {toc.assets.length > 0 && <li>{toc.assets.map(a => a.label).join(' + ')} to keep</li>}
          <li>Free updates for every future edition</li>
        </ul>

        {!entitled && (
          <div className="mt-6 flex flex-wrap items-center gap-4 rounded-lg border border-fire-200 bg-fire-50 p-5">
            <div className="flex-1 min-w-[240px]">
              <p className="text-lg font-semibold text-warm-900">{MANUAL_PRICE_LABEL} · one time</p>
              <p className="text-sm text-warm-700">
                Every chapter, and every future revision. Buy once — updates are free forever.
              </p>
            ...
            {isSignedIn ? (
              <button
                onClick={handleBuy}
                disabled={buying}
                className="rounded-md bg-fire-500 px-6 py-3 font-semibold text-white hover:bg-fire-600 disabled:opacity-60"
              >
                {buying ? 'Opening checkout…' : `Get the manual — ${MANUAL_PRICE_LABEL}`}
              </button>
            ) : (
              <SignInButton mode="modal">
                <button className="rounded-md bg-fire-500 px-6 py-3 font-semibold text-white hover:bg-fire-600">
                  Sign in to buy
                </button>
              </SignInButton>
            )}
            <p className="w-full text-xs text-warm-700">
              {freeChapters.length > 0 && (
                <>
                  Not sure yet?{' '}
                  <button
                    onClick={() => openChapter(freeChapters[0].slug)}
                    className="text-fire-700 underline-offset-2 hover:underline"
                  >
              ...
              Already bought it? <a className="text-fire-700 underline-offset-2 hover:underline" href="/products/session-manager/my-manual">Find your purchase →</a>
            </p>
          </div>
        )}
        ...
        {entitled && toc.assets.length > 0 && (
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <span className="text-sm text-emerald-700">✓ You own this manual</span>
            {toc.assets.map(a => (
              <button
                key={a.id}
                onClick={() => handleDownload(a.id)}
                disabled={downloading === a.id}
                className="rounded-md border border-warm-200 px-4 py-2 text-sm text-warm-900 hover:border-emerald-600 disabled:opacity-60"
              >
                {downloading === a.id ? 'Downloading…' : <>↓ {a.label} <span className="text-warm-700">({formatBytes(a.bytes)})</span></>}
              </button>
            ))}
          </div>
        )}
            ...
            <div className="rounded-lg border border-warm-200 bg-white p-8">
              ...
              {chapter.signedIn ? (
                <button
                  onClick={handleBuy}
                  disabled={buying}
                  className="mt-4 rounded-md bg-fire-500 px-5 py-2.5 font-semibold text-white hover:bg-fire-600 disabled:opacity-60"
                >
                  {buying ? 'Opening checkout…' : `Unlock for ${chapter.priceLabel}`}
                </button>
              ) : (
                <SignInButton mode="modal">
                  <button className="mt-4 rounded-md bg-fire-500 px-5 py-2.5 font-semibold text-white hover:bg-fire-600">
                    Sign in to unlock
                  </button>
                </SignInButton>
              )}
              {freeChapters.length > 0 && (
                <p className="mt-4 text-sm text-warm-700">
                  ...
                  <button onClick={() => openChapter(freeChapters[0].slug)} className="text-fire-700 underline-offset-2 hover:underline">
            ...
            <nav className="mt-12 flex flex-wrap gap-3 border-t border-warm-200 pt-6 text-sm">
              {prev && (
                ...
                >
                  ...
                  {prev.title}{!entitled && !prev.free && <span className="ml-2 text-warm-500">🔒</span>}
                </button>
              )}
              {next && (
                <button
                ...
                >
                  ...
                  {next.title}{!entitled && !next.free && <span className="ml-2 text-warm-500">🔒</span>}
                </button>
              )}
            </nav>
          )}
